			...
			case 1:
				...
				for (ReviewVO reviewVO : list) {
					System.out.println(String.format("%d번 | 작성일자: %s | 영화 제목: %s (아이디: %s)\n제목: %s | 평점: %s\n내용: %s\n",
							reviewVO.getReviewNum(), reviewVO.getReviewDate().toString().split(" ")[0],
							reviewVO.getMvTitle(), reviewVO.getCustomerId(), reviewVO.getReviewTitle(),
							reviewVO.getReviewRating(), reviewVO.getReviewContent()));
				}
				System.out.println("------------------------------------------------------------");
				break;
